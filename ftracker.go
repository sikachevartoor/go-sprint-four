...
const (
	walkingCaloriesWeightMultiplier float64 = 0.035 // множитель массы тела.
	walkingSpeedHeightMultiplier    float64 = 0.029 // множитель роста.
)
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	// ваш код здесь
	sqrMeanSpeedMS := math.Pow((meanSpeed(action, duration) * kmhInMsec), 2) // квадрат средней скорости в м/с
	heightM := height / cmInM                                                // рост в метрах
	return ((walkingCaloriesWeightMultiplier*weight + (sqrMeanSpeedMS/heightM)*walkingSpeedHeightMultiplier*weight) * duration * float64(minInH))
}
